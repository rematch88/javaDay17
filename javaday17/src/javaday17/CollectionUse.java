...
public class CollectionUse {
	...
	public static void main(String[] args) {
		...
		//인덱스를 이용해서 하나씩 접근 - 배열이나 List의 데이터 개수를 알아야합니다.
		...
		//데이터베이스나 C++에서는 이런 방법으로 접근하지만 Java나 Python 같은 언어는 다른 방법을 제공
		...
		//python에서의 numpy의 ndarray나 pandas의 Series, DataFrame 등의 동작방식이 이와 동일
		stream.forEach((name)->{System.out.println(name);});
		
		System.out.println();
		System.out.println("배열을 이용해서 스트림 생성"); // 실습할때는 자주 사용하지만 실제 프로그래밍에서는 대부분 List로 생성
		...
		try {
			...
			System.out.println("파일 내용 읽기 예외:" + e.getMessage());
			e.printStackTrace();
		}
